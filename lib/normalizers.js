...
export function normalizeJobOrderData(data) {
	...
	const ALLOWED_CURRENCIES = new Set(['USD', 'CAD', 'AUD']);
	const rawCurrency = typeof data.currency === 'string' ? data.currency.trim().toUpperCase() : '';
	const currencyCode = ALLOWED_CURRENCIES.has(rawCurrency) ? rawCurrency : 'USD';
	...
	return {
		...
		customFields: toNullableCustomFields(data.customFields),
		applicationQuestions: Array.isArray(data.applicationQuestions)
			? data.applicationQuestions.filter(
					(q) => q && typeof q.id === 'string' && typeof q.label === 'string' && q.label.trim()
				)
			: []
	};
}
